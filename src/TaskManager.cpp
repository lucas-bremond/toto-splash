...
///	@file 						TotoSplash/TaskManager.cpp
...
// #include <TotoSplash/AppSettings.hpp>
...
void							TaskManager::start							( )
{
	...
	timer_.initializeMs(100, Delegate<void()>(&TaskManager::onManage, this)) ; // TBM param
...
bool							TaskManager::addTask						(	const	Task&						aTask								)
{
	...
	if (tasks_.size() >= taskCountLimit_)
	{
		...
		// if (tasks_.firstElement().getStatus() != Task::Status::Pending) // Quickfix
		{
...
void							TaskManager::onManage						( )
{
	...
	for (uint idx = 0; idx < tasks_.size(); ++idx)
	{
		...
		if ((task.getStatus() == Task::Status::Pending) && (task.getExecutionTime() <= currentTime))
		{
		...
		else if ((task.getStatus() == Task::Status::Executing) && ((task.getExecutionTime() + task.getDuration()) <= currentTime))
		{
